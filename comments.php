...
<?php if ($comments) : //if there are comments to show, display this ?>
	...
	<h2 id="comments"><?php comments_number( __('Sin respuestas en', 'blank'), __('1 comentario en', 'blank'), __('% comentarios en', 'blank'));?> <em><?php the_title(); ?></em></h2>
	...
	<?php foreach ($comments as $comment) : ?>
		...
		<li <?php echo $oddcomment; ?>id="comment-<?php comment_ID() ?>">
			...
			<small class="commentmetadata"><a href="#comment-<?php comment_ID() ?>" title=""><?php comment_date('j F, Y') ?> <?php _e('a las', 'blank'); ?> <?php comment_time() ?></a> 
...
<p><?php _e('Debes estar', 'blank'); ?> <a href="<?php echo get_option('siteurl'); ?>/wp-login.php?redirect_to=<?php echo urlencode(get_permalink()); ?>"><?php _e('registrado', 'blank'); ?></a> <?php _e('para comentar', 'blank'); ?>.</p>
<?php else : ?>
...
<p><?php _e('Registrado como', 'blank'); ?> <a href="<?php echo get_option('siteurl'); ?>/wp-admin/profile.php"><?php echo $user_identity; ?></a>. <a href="<?php echo get_option('siteurl'); ?>/wp-login.php?action=logout" title="<?php _e('salir', 'blank'); ?>"><?php _e('salir', 'blank'); ?> &raquo;</a></p>

<?php else : ?>

		<p id="comment-notes"><?php _e('tu correo no ser&aacute; publicado', 'blank' ) ?>.<br />
		<?php if ($req) _e('Los campos con <span class="required">*</span> son obligatorios', 'blank' ) ?></p>
                             
<p>
<label for="author"><?php _e('nombre', 'blank' ) ?> <?php if ($req) echo '*'; ?></label><br />
...
<p>
<label for="email"><?php _e('e-mail', 'blank' ) ?> <?php if ($req) echo '*'; ?></label><br />
...
<p>
<label for="url"><?php _e('web', 'blank' ) ?></label><br />
...
<p><input name="submit" type="submit" id="submit" tabindex="5" value="<?php _e('enviar', 'blank') ?>" />
